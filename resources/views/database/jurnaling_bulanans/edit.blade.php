...
<form action='{{ url('/admin/jurnaling_bulanans/'.$data->id) }}' method='post' enctype="multipart/form-data">
@csrf
@method('PUT')

<div class="my-3 p-3 bg-body rounded shadow-sm">
    <a href="{{ url('/admin/templates-pastikan-ada-s') }}" class="btn btn-secondary"> << Kembali</a>
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <select class="form-control" name='periode' value="{{ $data->periode }}" id="periode">
                <option value="">Pilih periode</option>
                <option value="1" @if ($data->periode == "1") selected @endif>1</option>
                <option value="2" @if ($data->periode == "2") selected @endif>2</option>
            </select>
        </div>
    </div>
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <select class="form-control" name='id_penilai' value="{{ $data->id_penilai }}" id="id_penilai">
                ...
                @foreach ($pembimbing_lapangans as $pembimbing_lapangan)
                    <option value="{{ $pembimbing_lapangan->id }}" @if ($data->id_penilai == $pembimbing_lapangan->id) selected @endif>{{ $pembimbing_lapangan->nip_baru }} - {{ $pembimbing_lapangan->nama }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" name='uraian_kegiatan' value="{{ $data->uraian_kegiatan }}" id="uraian_kegiatan">
        </div>
    </div>
    {{-- SATUAN --}}
    <div class="mb-3 row">
   ...
   {{-- KUANTITAS TARGET --}}
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" name='kuantitas_target' value="{{ $data->kuantitas_target }}" id="kuantitas_target">
        </div>
    </div>
    {{-- KUANTITAS REALISASI --}}
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" name='kuantitas_realisasi' value="{{ $data->kuantitas_realisasi }}" id="kuantitas_realisasi">
        </div>
    </div>
    <div class="mb-3 row">
        <label for="tingkat_kuantitas" class="col-sm-2 col-form-label">Tingkat Kuantitas</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name='tingkat_kuantitas' value="{{ $data->tingkat_kuantitas }}" id="tingkat_kuantitas">
        </div>
    </div>
    <div class="mb-3 row">
        <label for="tingkat_kualitas" class="col-sm-2 col-form-label">Tingkat Kualitas</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name='tingkat_kualitas' value="{{ $data->tingkat_kualitas }}" id="tingkat_kualitas">
        </div>
    </div>
    <div class="mb-3 row">
