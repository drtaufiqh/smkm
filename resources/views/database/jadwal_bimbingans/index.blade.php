...
<div class="my-3 p-3 bg-body rounded shadow-sm">
      <!-- FORM PENCARIAN -->
      <div class="pb-3">
        <form class="d-flex" action="{{ url('jadwal_bimbingans') }}" method="get">
      ...
      <!-- TOMBOL TAMBAH DATA -->
      <div class="pb-3">
        <a href='{{ url('jadwal_bimbingans/create') }}' class="btn btn-primary">+ Tambah Data</a>
      </div>
          ...
          <tbody>
            ...
            @foreach ($data as $item)
              <tr class="align-middle">
                  ...
                  <td class="">
                      <a href='{{ url('jadwal_bimbingans/'.$item->id.'/edit') }}' class="btn btn-warning btn-sm">Edit</a>
                      <form onsubmit="return confirm('Yakin akan menghapus data?')" class="d-inline" action="{{ url('jadwal_bimbingans/'.$item->id) }}" method="post">
                        @csrf
                        @method("DELETE")
